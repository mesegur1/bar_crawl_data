...
class HdcLevelEncoder(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        fft_signals = torch.real(torch.fft.fft(input[:, 1:], dim=0))
        ...
        fft_mag_levels = self.signal_level_mag_fft(self.calc_mags(fft_signals))
        energy_fft_levels = self.signal_level_energy_fft(self.calc_energy(fft_signals))
        ...
        sample_hvs = (
            ...
            + (x_fft_levels * y_fft_levels * z_fft_levels + fft_mag_levels + energy_fft_levels)
        ) * times
