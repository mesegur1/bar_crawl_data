...
class HdcRbfEncoder(torch.nn.Module):
    def __init__(self, timestamps: int, out_dimension: int, use_tanh: bool = True):
        ...
        if use_tanh == True:
            self.kernel1 = embeddings.HyperTangent(
                timestamps * 3, out_dimension, dtype=torch.complex64
            )
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if window < self.timestamps:
            ...
            x_fft_signal = F.pad(
                input=fft_signals[:, 0], pad=(0, padding), mode="constant", value=0
            )
            y_fft_signal = F.pad(
                input=fft_signals[:, 1], pad=(0, padding), mode="constant", value=0
            )
            z_fft_signal = F.pad(
                input=fft_signals[:, 2], pad=(0, padding), mode="constant", value=0
            )
        else:
            x_signal = input[:, 1]
            y_signal = input[:, 2]
            z_signal = input[:, 3]
            x_fft_signal = fft_signals[:, 0]
            y_fft_signal = fft_signals[:, 1]
            z_fft_signal = fft_signals[:, 2]
        features1 = torch.cat((x_signal, y_signal, z_signal)).to(dtype=torch.complex64)
        features2 = torch.cat((x_fft_signal, y_fft_signal, z_fft_signal)).to(
            dtype=torch.complex64
        )
        # Use kernel encoder
        sample_hv1 = self.kernel1(features1)
        sample_hv2 = self.kernel2(features2)
        sample_hv = sample_hv1 + sample_hv2
