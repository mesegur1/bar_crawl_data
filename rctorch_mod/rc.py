...
from .activations import (
    _nrmse,
    _convert_activation_f,
    _sech2,
    _sigmoid_derivative,
    _sinsq,
    _inverse_hyperbolic_tangent,
    _sech2_,
    _identity,
    _neg_sin,
    _neg_double_sin,
...
class RcNetwork(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
    ):  # <-- this line is backprop arguments #beta = None
        super().__init__()
        ...
        acceptable_args = [
            ...
            "enet_strength",
            "feedback",
            "feedback_connectivity",
            "feedback_scaling",
            "feedback_weight_dist",
            "gamma",
            "gamma_cyclic",
            "id_",
            "input_connectivity",
        ...
        if self.reservoir_weight_dist == "uniform":
            if isinstance(self.mu, float) or isinstance(self.sigma, float):
                if self.mu != 0:
                    assert (
                        False
                    ), "to use mu and sigma hps use reservoir_weight_dist = 'normal'"

        # random state and default tensor arguments
        self.random_state = torch.Generator(device=self.device).manual_seed(
            random_state
        )
        ...
        if type(activation_function) == str:
            ...
            self.activation_function, self.act_f_prime = _convert_activation_f(
                activation_function
            )
            ...
            self._act_fs = [
                _convert_activation_f(act_f, derivative=False, both=False)
                for act_f in activation_function
            ]
            self._act_f_primes = [
                _convert_activation_f(act_f, derivative=False, both=False)
                for act_f in activation_function
            ...
            n_fs = list(range(len(self._act_fs)))
            self._act_mask = torch.tensor(
                np.random.choice(n_fs, size=self.n_nodes, p=probs)
            )
        else:
        ...
        try:
            ...
            assert (
                False
            ), f"You must enter either 'regularization' or 'log_regularization as an argument to the RcNetwork.__init__ method'"
    ...
    def __repr__(self):
        ...
        strr = (
            "{"
            + f"n_nodes : {n_nodes}, connectivity : {connect}, spectral_radius : {spect}"
            + "}"
        )
        ...
        with torch.no_grad():
            ...
            if self.ODE_order:
                ...
                if len(init_conditions) > 1:
                    if (
                        type(init_conditions[1]) == list
                        or type(init_conditions[1]) == np.ndarray
                    ):
                        ...
                        for i in range(len(init_conditions)):
                            init_conditions[i] = np.random.uniform(
                                low=init_conditions[i][0], high=init_conditions[i][1]
                            )
                        init_conditions[0] = [init_conditions[i]]
                    elif (
                        type(init_conditions[0]) == list
                        or type(init_conditions[0]) == np.ndarray
                    ):
                        multiple_ICs = self.multiple_ICs = True
                    else:
                ...
                elif type(init_conditions[0]) in [
                    float,
                    int,
                    np.float64,
                    np.float32,
                    ...
                    assert (
                        False
                    ), f"malformed ICs, either use a list, a float or an integer"
                ...
                if self.dt == None:
                    ...
                    if dt_speculative != 0:
                        ...
                        print(
                            f"assuming dt is {dt_speculative}, if this is incorrect please override dt."
                        )
                ...
                if self.dt != None:
                    ...
                    if not random_sampling:
                        ...
                        X = (
                            torch.linspace(
                                start, stop, steps=nsteps, requires_grad=False
                            )
                            .view(-1, 1)
                            .to(self.device)
                        )
                    ...
                    X = (
                        torch.linspace(x0, xf, steps=nsteps, requires_grad=False)
                        .view(-1, 1)
                        .to(self.device)
                    )
                else:
                ...
                if random_sampling:
                    ...
                    idx = np.random.choice(
                        range(X.shape[0]), sample_timepoints, replace=False
                    )
                    idx.sort()
            ...
            if self.unscaled_X.std() != 0:
                ...
                if SCALE:
                    self.X.data = self._scale(
                        inputs=self.unscaled_X, keep=True, normalize=False
                    )  # .clone()
            else:
            ...
            if self.ODE_order and self.burn_in:
                ...
                y_extended = torch.ones((self.X_extended.shape[0], y.shape[1])).to(
                    self.device
                )
            else:
                y_extended = y
            ...
            if self.noise is not None:
                self.noise_z = (
                    torch.normal(
                        0, 1, size=(self.n_nodes, self.X_extended.shape[0]), **self.dev
                    )
                    * self.noise
                )
            ...
            with torch.set_grad_enabled(self.track_in_grad):
                ...
                if out_weights:
                    ...
                    self.LinOut.weight = Parameter(
                        out_weights["weights"].to(self.device)
                    )
                    ...
                    try:
                        ...
                        self.LinOut.weight = Parameter(
                            self.LinOut.weight.to(self.device)
                        )
                ...
                self.states = self.train_states(
                    self.X_extended, y_extended, self.states
                )
                ...
                if self.ODE_order:
                    if self.feedback:
                        ...
                        updates = (
                            self.LinIn(input_) + self.bias + self.LinRes(self.states)
                        )
                    else:
                        try:
                            updates = (
                                self.LinIn(self.X)
                                + self.bias
                                + self.LinRes(self.states)
                            )
                        except:
                            assert (
                                False
                            ), f"X_extended {self.X_extended.shape}, X {self.X.shape}, LinIn {self.LinIn.weight.shape}, self.LinRes {self.LinRes.weight.shape}, states {self.states.shape} "

                    self.states_dot = (
                        -self.alpha * self.states
                        + self.alpha * self.activation_function(updates.T).T
                    )
                    self.states_dot = torch.cat(
                        (torch.ones_like(self.X), self.states_dot), axis=1
                    )
                ...
                self.sb = states_with_bias = torch.cat(
                    (
                        torch.ones_like(self.extended_states[:, 0].view(-1, 1)),
                ...
                if self.ODE_order:
                    self.sb1 = states_dot_with_bias = torch.cat(
                        (
                            torch.zeros_like(self.states_dot[:, 0].view(-1, 1)),
                            self.states_dot,
                        ),
                        axis=1,
                    )
                        ...
                        with torch.set_grad_enabled(False):
                            if self.ODE_order:
                                ...
                                if eq_system:
                                    ...
                                    self.Sigma = (
                                        Sigma
                                    ) = sigma1 + self.regularization * torch.eye(
                                        sigma1.shape[1], **self.dev
                                    )
                                    ...
                                    if not multiple_ICs:
                                        self.Wy = Wy = (
                                            Lam.T
                                            @ (Cx @ delta_inv + Cp @ sigma_inv).T
                                            / len(self.X)
                                        )
                                        self.Wp = Wp = (
                                            Lam.T
                                            @ (Cp @ delta_inv - Cx @ sigma_inv).T
                                            / len(self.X)
                                        )

                                        self.weight = weight = torch.cat(
                                            (Wy.view(-1, 1), Wp.view(-1, 1)), axis=1
                                        )
                                    ...
                                    if multiple_ICs:
                                        if nl:
                                            ...
                                            p = self.ode_coefs[0]
                                            self.D_As = D_As = [
                                                y0 * (p + y0 * self.q) - self.force_t
                                                for y0 in y0s
                                            ]
                                        else:
                                            D_As = [
                                                A * ode_coefs[0] - self.force_t
                                                for A in As
                                            ]
                                    else:
                                        if nl:
                                    ...
                                    if nl:
                                        ...
                                        # assert False
                                        self.DH = DH = (
                                            ode_coefs[0] * gH + ode_coefs[1] * gH_dot
                                        )
                                    ...
                                    if not nl:
                                        ...
                                        self.DH1 = (
                                            DH1
                                        ) = DH1 + self.regularization * torch.eye(
                                            DH1.shape[1], **self.dev
                                        )
                                        ...
                                        self.DH2s = DH2s = [
                                            DHinv @ DHs[i].T
                                            for i, DHinv in enumerate(DHinvs)
                                        ]
            ...
            if multiple_ICs:
                if not backprop_f:
                    ...
                    if self.parallel_backprop:
                        ...
                        if SOLVE:
                            new_out_W.weight = Parameter(
                                self.weights_list[i].view(self.n_outputs, -1)
                            ).requires_grad_(True)
                            new_out_W.bias = Parameter(
                                self.biases_list[i].view(1, self.n_outputs)
                            ).requires_grad_(True)
                        else:
                            try:
                                new_out_W.weight = Parameter(
                                    orig_weights.view(self.n_outputs, -1)
                                ).requires_grad_(True)
                                new_out_W.bias = Parameter(
                                    orig_bias.view(1, self.n_outputs)
                                ).requires_grad_(True)
                            except:
                                new_out_W.weight = Parameter(
                                    orig_weights.reshape(self.n_outputs, -1)
                                ).requires_grad_(True)
                                new_out_W.bias = Parameter(
                                    orig_bias.reshape(1, self.n_outputs)
                                ).requires_grad_(True)
                        ...
                        data2save = {  # "rc" : self,
                            ...
                            "states_dot": self.states_dot.detach().requires_grad_(
                                False
                            ),
                            # "orig_bias" : orig_bias,
                        ...
                        weight_dicts = ray.get(
                            [
                                execute_backprop.remote(self_id, y0)
                                for y0 in init_conds_clone[0]
                            ]
                        )
                        ...
                        for i, y0 in enumerate(init_conds_clone[0]):
                            ...
                            with torch.enable_grad():
                                weight_dict = backprop_f(
                                    self,
                                    force_t=self.force_t,
                                    custom_loss=ODE_criterion,
                                    epochs=epochs,
                                )
                    ...
                    for weight_dict in weight_dicts:
                        ...
                        loss, gd_weight, gd_bias = (
                            weight_dict["loss"]["loss_history"],
                            weight_dict["weights"],
                            weight_dict["bias"],
                        )
                        scores.append(score)
    ...
    def error(self, predicted, target, method="nmse", alpha=1.0):
        """
        ...
        def nmse(y, yhat):
            ...
            """
            return (
                (torch.sum(torch.square(y - yhat)) / torch.sum(torch.square(y)))
            ) / len(y.squeeze())
        ...
        if type(method) != type("custom"):
            ...
            nmse = torch.mean(torch.square(errors)) / torch.square(
                target.squeeze().std()
            )
            ...
            kl = torch.sigmoid(
                torch.exp(
                    torch.nn.KLDivLoss(reduction="sum")(
                        torch.softmax(predicted, dim=-1), torch.softmax(target, dim=-1)
                    )
                )
            )
            error = nmse + kl
            ...
            error = alpha * torch.tanh(
                torch.mean(torch.square(errors)) / alpha
            )  # To 'squeeze' errors onto the interval (0, 1)
        elif method == "rmse":
            error = torch.sqrt(torch.mean(torch.square(errors)))
        elif method == "nmse":
            error = torch.mean(torch.square(errors)) / torch.square(
                target.squeeze().std()
            )  # ddof=1))
        elif method == "nrmse":
            error = (
                torch.sqrt(torch.mean(torch.square(errors))) / target.flatten().std()
            )  # ddof=1)
        elif method == "tanh-nrmse":
            nrmse = torch.sqrt(torch.mean(torch.square(errors))) / target.flatten().std(
                ddof=1
            )
            ...
            error = torch.log(alpha * torch.tanh((1.0 / alpha) * nrmse))
        else:
    ...
    def test(
        ...
        if not self.ODE_order:
            ...
            if self.dt != None:
                start, stop = float(X[0]), float(X[-1])
                nsteps = int((stop - start) / self.dt)
                X = torch.linspace(start, stop, steps=nsteps, requires_grad=False).view(
                    -1, 1
                )  # .to(self.device)
            elif type(X) == type([]) and len(X) == 3:
                x0, xf, nsteps = X  # 6*np.pi, 100
                X = torch.linspace(x0, xf, steps=nsteps, requires_grad=False).view(
                    -1, 1
                )  # .to(self.device)
            final_t = X.shape[0]
        ...
        if steps_ahead is None:
            if not self.ODE_order:
                self._pred = self.predict(
                    n_steps=y.shape[0], X=X, y_start=y_start, scale=True
                )

                score = self.error(
                    predicted=self._pred, target=y, method=scoring_method, alpha=alpha
                )
                if self.id_ == None:
                    return (
                        score,
                        self._pred.data,
                    )  # {"yhat": y_predicted.data, "ytest": y}, X[self.burn_in:]
                else:
                    score.detach()
                    self._pred.detach()
            ...
            y_predicted = self.predict_stepwise(
                y, X, steps_ahead=steps_ahead, y_start=y_start
            )[:final_t, :]
    ...
    def predict(
        ...
        if not X is None:
            if scale and self.unscaled_X.std() != 0:
                self.X_val = Parameter(
                    self._scale(inputs=self.unscaled_Xte, normalize=False)
                )
            else:
        ...
        if self.noise is not None:
            self.noise_z = (
                torch.normal(0, 1, size=(self.n_nodes, self.X_val.shape[0]), **self.dev)
                * self.noise
            )
        ...
        if not y_start is None:
            lastoutput = self._scale(
                outputs=y_start, normalize=self._normalize
            )  # self.scale(inputs=X)

        inputs = torch.vstack([lastinput, self.X_val_extended]).view(
            -1, self.X_val_extended.shape[1]
        )
        ...
        with torch.no_grad():
            if self.ODE_order:
                ...
                states, outputs = self._unsupervised_val_states(
                    n_samples, inputs, states, outputs
                )

            else:
                # states = torch.vstack((states, states))
                states, outputs = self._supervised_val_states(
                    n_samples, inputs, states, outputs
                )
            ...
            if not self.ODE_order:
                # try:
                #     if scale:
                yhat = self._descale(outputs=outputs, normalize=self._normalize).view(
                    -1, self.n_outputs
                )
                # except:
                #     yhat = outputs
                ...
                time = self.X_val
                states_dot = -self.alpha * states[
                    :, 1:
                ] + self.alpha * self.activation_function(updates)
                # if self.ODE_order == 2:
                ...
                states_dot = torch.cat(
                    (torch.ones_like(self.X_val), states_dot), axis=1
                )
                ...
                assert type(self.reparam_f) != type(
                    None
                ), "you must input a reparam function with ODE"
    ...
    def plot_prediction(
        ...
        if not self._override:
            ...
            plt.plot(
                ...
                self._pred.cpu(),
                alpha=pred_alpha,
                linewidth=lw + 2,  # color = "red",
                label="test",
            )
            try:
                input_train, input_test = list(input_train.ravel()), list(
                    input_test.ravel()
                )
            except:
                pass
            ...
            plt.plot(
                ...
                np.concatenate((self._gt_tr.cpu(), self._gt_te.cpu()), axis=0),
                "--",
                color="black",
                alpha=gt_alpha,
                linewidth=lw - 1,
    ...
    def plot_residuals(
        ...
        plt.plot(input_test, self.te_resids.cpu())
            ...
            if normalize:
                ...
                if not self.ODE_order:
                    ...
                    if self.output_activation == "sin":
                        normalized = (
                            ((inputs - self._input_mins) / self._input_ranges) - 0.5
                        ) * 2
                    else:
                        normalized = (
                            ((inputs - self._input_mins) / self._input_ranges) - 0.5
                        ) * self.tanh_bound_limit
        ...
        if not outputs is None:
            if normalize:
                ...
                if not self.ODE_order:
                    # normalize to between -0.5 and 0.5 for echostate purposes
                    normalized = (
                        ((outputs - self._output_mins) / self._output_ranges) - 0.5
                    ) * self.tanh_bound_limit
                ...
                if self.ODE_order:
                    ...
                    transformed.append(
                        (outputs - self._output_means) / self._output_stds
                    )
    ...
    def _descale(self, inputs=None, outputs=None, normalize=False):
        ...
        if normalize:
            ...
            if not inputs is None:
                assert (
                    False
                ), f" there is no need to normalize the input, standardize instead"
            ...
            if not outputs is None:
                ...
                if self.output_activation == "sin":
                    denormalized = (
                        self.output_f(destandardized / 2) + 0.5
                    ) * self._output_ranges + self._output_mins

                else:
                    denormalized = (
                        self.output_f(destandardized / self.tanh_bound_limit) + 0.5
                    ) * self._output_ranges + self._output_mins
            ...
            if not outputs is None:
                if self.ODE_order:
                    ...
                    transformed.append(
                        (outputs * self._output_stds) + self._output_means
                    )
    ...
    def _plot_prep(self, gt_tr_override=None, gt_te_override=None):
        ...
        self.tr_resids = (self._gt_tr.cpu() - self.yfit) ** 2
    ...
    def _train_state_feedback(self, t, X, state, y, output=False, retain_grad=False):
        ...
        preactivation = self.preactivation(
            t, input_vector, recurrent_vec, self.bias
        )  # , self.beta)
    ...
    def _train_state_feedback_unsupervised(
        self, t, X, state, y, output=False, retain_grad=False
    ):
        """
        Arguments:
            t: the current timestep
    ...
    def _unsupervised_val_states(self, n_samples, inputs, states, outputs):
        ...
        for t in range(n_samples):
            ...
            state_t, output_t = self.train_state(
                t, X=input_t, state=state_t, y=output_prev
            )
    ...
    def _gen_uniform_weights(self, dim, connectivity=None):
        ...
        n, m = dim[0], dim[1]
        weights = torch.rand(
            n, m, generator=self.random_state, device=self.device, requires_grad=False
        )
        weights = (weights * 2) - 1
    ...
    def _gen_reservoir(self, obs_idx=None, targ_idx=None, load_failed=None):
        ...
        for i in range(max_tries):
            ...
            if not self.reservoir or not self.approximate_reservoir or load_failed == 1:
                self.accept = (
                    torch.rand(self.n_nodes, self.n_nodes, **self.tensor_args)
                    < self.connectivity
                )

                if self.reservoir_weight_dist == "uniform":
                    self.weights = (
                        torch.rand(self.n_nodes, self.n_nodes, **self.tensor_args) * 2
                        - 1
                    )
                elif self.reservoir_weight_dist == "normal":
                    shape_tuple = (self.n_nodes, self.n_nodes)
                    ones_tensor, zeros_tensor = torch.ones(
                        shape_tuple, **self.dev
                    ), torch.zeros(shape_tuple, **self.dev)
                    self.weights = (
                        torch.normal(mean=ones_tensor, std=zeros_tensor) * self.sigma
                        + self.mu
                    )
                elif self.reservoir_weight_dist == "discrete":
                    dim = (self.n_nodes, self.n_nodes)
                    self.weights = self._gen_discrete_weights(
                    ...
                    assert (
                        False
                    ), "{self.reservoir_weight_dist} reservoir_weight_distribution not yet implimented"
                ...
                try:
                    if self.approximate_reservoir:
                        self.weights = self.reservoir.get_approx_preRes(
                            self.connectivity, i
                        ).to(self.device)
                    else:
                        self.weights = (
                            self.reservoir.reservoir_pre_weights < self.connectivity
                        )
                        self.weights *= self.reservoir.accept
                        self.weights = self.weights

                        del self.accept
                        del self.reservoir.reservoir_pre_weights
                    ...
                    if i > self.reservoir.number_of_preloaded_sparse_sets:
                        load_failed = 1
                        _printc(
                            "All preloaded reservoirs Nilpotent, generating random reservoirs, connectivity ="
                            + str(round(self.connectivity, 8))
                            + "...regenerating",
                            "fail",
                        )
                    continue
            ...
            max_eigenvalue = (
                torch.linalg.eigvals(self.weights).abs().max()
            )  # .type(torch.float32) .sort(descending = True).values.
            ...
            if max_eigenvalue > 0:
                ...
                if not already_warned:
                    _printc(
                        "Loaded Reservoir is Nilpotent (max_eigenvalue ={}), connectivity ={}.. .regenerating".format(
                            max_eigenvalue, round(self.connectivity, 8)
                        ),
                        "fail",
                    )
                already_warned = True
                ...
                if i == max_tries - 1:
                    raise ValueError(
                        "Nilpotent reservoirs are not allowed. Increase connectivity and/or number of nodes."
                    )
        ...
        if load_failed == 1 or not self.reservoir:
            self.state = torch.zeros(
                1, self.n_nodes, device=torch_device(self.device), **self.no_grad_
            )
        else:
        ...
        with torch.no_grad():
            ...
            if not self.reservoir or "in_weights" not in dir(self.reservoir):
                ...
                if self.input_weight_dist == "uniform":
                    in_weights = self._gen_uniform_weights(
                        dim=(n, m), connectivity=self.input_connectivity
                    )
                    ...
                    in_weights = self._gen_discrete_weights(
                        sigma=1, dim=(n, m), connectivity=self.input_connectivity
                    )

                else:
                    assert (
                        False
                    ), f"input_weight_dist {self.input_weight_dist} not implimented, try uniform or discrete."

            else:
                in_weights = (
                    self.reservoir.in_weights
                )  # + self.noise * self.reservoir.noise_z One possibility is to add noise here, another is after activation.

                ##### Later for speed re-add the feedback weights here.

                if self.feedback:
                    feedback_weights = (
                        self.feedback_scaling * self.reservoir.feedback_weights
                    )
            ...
            if self.feedback:
                if self.feedback_weight_dist == "uniform":
                    feedback_weights = self._gen_uniform_weights(
                        dim=(n, o), connectivity=self.feedback_connectivity
                    )  # torch.rand(self.n_nodes, self.n_outputs, device = self.device, requires_grad = False, generator = self.random_state) * 2 - 1
                    ...
                    # input_scaling is input sigma.
                    feedback_weights = self._gen_discrete_weights(
                        sigma=1, dim=(n, o), connectivity=self.feedback_connectivity
                    )

                else:
                    assert (
                        False
                    ), f"feedback weight dist {self.feedback_weight_dist} not implimented. Try uniform or discrete"
    ...
    def _train_states_supervised(
        self, X, y, states, outputs: bool = True
    ):  # calc_grads : bool = True,
        """
        Summary line.
        ...
        with torch.no_grad():
            for t in range(1, X.shape[0]):
                # print(t)
                # input_t =  X[t, :].T
                input_t = X[t, :].permute(*torch.arange(X[t, :].ndim - 1, -1, -1))
                state_t, _ = self.train_state(
                    t, X=input_t, state=states[t - 1, :], y=y[t - 1, :]
                )
    ...
    def _train_states_unsupervised(self, X, y, states):
        ...
        with torch.no_grad():
            ...
            for t in range(1, X.shape[0]):
                ...
                state_t, output_t = self.train_state(
                    t, X=input_t, state=states[t - 1, :], y=y[t - 1, :], output=False
                )
    ...
    def _extend_X(self, X, random_sampling):
        ...
        if self.burn_in and self.ODE_order and not random_sampling:
            ...
            neg_X = (
                torch.linspace(start, float(X[0]), self.burn_in)
                .view(-1, 1)
                .to(self.device)
            )
    ...
    def _solve_supervised(self, y, return_states, SCALE):
        ...
        ridge_x = torch.matmul(train_x.T, train_x) + self.regularization * torch.eye(
            train_x.shape[1], **self.dev
        )
        ...
        if return_states:
            return (
                self.extended_states,
                (y[1:, :] if self.feedback else y),
                self.burn_in,
            )
        else:
            yfit_norm = self.LinOut(
                self.extended_states
            )  # self.LinOut.weight.cpu()@self.extended_states.T.cpu() + self.LinOut.bias.cpu()
            # yfit = self._output_stds.cpu()* (yfit_norm)+ self._output_means.cpu()

            self.yfit = (
                self._descale(outputs=yfit_norm, normalize=self._normalize)
                .view(-1, self.n_outputs)
                .detach()
                .cpu()
                .numpy()
            )
