...
class RcNetwork(nn.Module):
    ...
    def plot_prediction(self, axis_label_fontsize = None, 
        ...
        if not self._override:
            ...
            plt.plot(input_test, self._pred.cpu(), alpha = pred_alpha, linewidth = lw+2, #color = "red", 
                    label = "test")
            try:
    ...
    def _solve_supervised(self, y, return_states, SCALE):
        ...
        #     return self.yfit
        if return_states:
            return self.extended_states, (y[1:,:] if self.feedback else y), self.burn_in
        else:
            ...
            self.yfit = self._descale(outputs = yfit_norm, normalize = self._normalize).view(-1, self.n_outputs).detach().cpu().numpy()
            return self.yfit